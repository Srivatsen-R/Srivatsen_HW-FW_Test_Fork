...
#define ARBITRATION_ID_OFFSET -0x1
...
void isotp_continue_receive(IsoTpShims* shims, IsoTpMessage* message,
    ...
    switch(pci) {
        ...
        case PCI_FIRST_FRAME: {
            ...
            memcpy(handle->receive_buffer, &data[2], CAN_MESSAGE_BYTE_SIZE - 2);
            // memory_copy(handle->receive_buffer, &data[2], CAN_MESSAGE_BYTE_SIZE - 2);
